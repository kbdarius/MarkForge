...
exports.ensureMarkdownEngine = exports.renderHTML = exports.renderPage = void 0;
...
function getSciprts() {
    ...
    // Mermaid: inject local mermaid.js + converter for 'language-mermaid' code blocks.
    // Path is read from the 'markdown-pdf.mermaidServer' workspace setting (file:/// URL).
    try {
        let mermaidSetting = vscode.workspace.getConfiguration('markdown-pdf').get('mermaidServer');
        if (mermaidSetting && typeof mermaidSetting === 'string' && mermaidSetting.startsWith('file:///')) {
            let mermaidFile = decodeURIComponent(mermaidSetting.replace(/^file:\/\/\//, '')).replace(/\//g, require('path').sep);
            if (fs.existsSync(mermaidFile)) {
                scripts.push("");
                scripts.push("<!-- mermaid start -->");
                scripts.push(`<script type="text/javascript" src="${dataUri_1.fileToDataUri(mermaidFile)}"></script>`);
                scripts.push(`<script>mermaid.initialize({startOnLoad:false,theme:'default',securityLevel:'loose'});<\/script>`);
                scripts.push(`<script>document.addEventListener('DOMContentLoaded',function(){var nodes=[];document.querySelectorAll('pre>code.language-mermaid').forEach(function(el){var d=document.createElement('div');d.className='mermaid';d.textContent=(el.innerText||el.textContent).trim();el.parentNode.replaceWith(d);nodes.push(d);});if(nodes.length>0)mermaid.run({nodes:nodes});});<\/script>`);
                scripts.push("<!-- mermaid end -->");
            }
        }
    } catch(e) { /* mermaid not configured or unavailable */ }
    scripts.push("");
    return scripts.join('\n');
}
